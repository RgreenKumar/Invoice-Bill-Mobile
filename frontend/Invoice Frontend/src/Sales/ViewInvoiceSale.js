...
const ViewSaleInvoices = () => {
  const token = sessionStorage.getItem("token");
  const MySwal = withReactContent(Swal);
  const canCreate = getPermission(MODULES.SALE_INVOICE, "canCreate");
  const canEdit   = getPermission(MODULES.SALE_INVOICE, "canEdit");
  const canDelete = getPermission(MODULES.SALE_INVOICE, "canDelete");
  ...
  const [filterMonth, setFilterMonth] = useState("All Sale Invoices");
  ...
  const monthOptions = ["All Sale Invoices", "This Month", "Last Month", "This Year"];
  ...
  const fetchInvoices = async () => {
    try {
      ...
      // ✅ Always send date — for "All Sale Invoices" send wide range
      if (filterMonth === "All Sale Invoices") {
      ...
      params.append("billTypes", "SALE,POS");
  ...
  // ── FETCH MY COMPANY (for View modal header) ────────
  useEffect(() => {
  ...
  const handleColumnSearch = (e) => {
    setColumnSearch({ ...columnSearch, [e.target.name]: e.target.value });
  };
  ...
  const applyDateRange = (option) => {
    ...
    if (option === "This Month") {
      ...
      setShowCalendar(false);
    } else if (option === "All Sale Invoices") {
  ...
  // Close month + calendar dropdowns on outside click
  useEffect(() => {
    const handler = (e) => {
  ...
  const handleView = async (id) => {
    ...
    try {
      ...
      MySwal.fire("Error", "Failed to load invoice.", "error");
  ...
  const handleDownload = async (id, invoiceNumber) => {
    ...
    try {
      ...
      MySwal.fire("Error", "Failed to download invoice.", "error");
    } finally {
          ...
          <div className="card invoice-card">
            <div className="card-body p-0">
              ...
              <div className="d-flex align-items-center justify-content-between px-3 py-2 border-bottom">
                <h5 className="mb-0 font-weight-bold" style={{ fontSize: "16px" }}>Sale Invoices</h5>
                 {canCreate && (
                <div className="d-flex align-items-center" style={{ gap: "8px" }}>
              ...
              >
                ...
                <div ref={calendarRef} style={{ position: "relative" }}>
                  <div
                    className="d-flex align-items-center"
                    onClick={() => { if (filterMonth === "All Sale Invoices") setShowCalendar((p) => !p); }}
                    style={{ border: "1px solid #ddd", borderRadius: "20px", padding: "3px 12px", background: "#fff", cursor: filterMonth === "All Sale Invoices" ? "pointer" : "default", gap: "6px", fontSize: "13px" }}
                  >
                  ...
                  {showCalendar && filterMonth === "All Sale Invoices" && (
              ...
              <div className="px-3 pt-3">
                <div style={{ border: "1px solid #e0e0e0", borderRadius: "10px", padding: "14px 18px", background: "#fff", maxWidth: "300px", boxShadow: "0 1px 4px rgba(0,0,0,0.05)" }}>
                  <div className="d-flex justify-content-between align-items-start mb-1">
                    <span style={{ fontSize: "13px", color: "#888" }}>Total Sales Amount</span>
                  </div>
              ...
              {loading ? (
                ...
                <div className="d-flex flex-column align-items-center justify-content-center" style={{ paddingTop: "30px", paddingBottom: "50px" }}>
                  ...
                  <p style={{ fontSize: "13px", color: "#aaa", marginBottom: "18px" }}>You haven't added any transactions yet.</p>
                  <button
                    className="btn btn-danger"
                    onClick={() => navigate("/addsale")}
                    style={{ borderRadius: "8px", fontWeight: "600", padding: "8px 24px", fontSize: "14px" }}
                  >
                    <i className="fa-solid fa-plus mr-2"></i> Add Sale
                  </button>
                </div>
                  ...
                  <table className="table table-hover table-bordered table-sm invoice-table" style={{ fontSize: "13px" }}>
                   <thead style={{ background: "#222", color: "#fff" }}>
                    <tr>
                      ...
                      <th style={{ fontSize: "12px", fontWeight: "600", padding: "10px 12px" }}>Invoice No.</th>
                      ...
                      <th style={{ fontSize: "12px", fontWeight: "600", padding: "10px 12px" }}>Payment Type</th>
                      <th style={{ fontSize: "12px", fontWeight: "600", padding: "10px 12px" }}>Amount (₹)</th>
                      <th style={{ fontSize: "12px", fontWeight: "600", padding: "10px 12px" }}>Balance (₹)</th>
                      <th style={{ fontSize: "12px", fontWeight: "600", padding: "10px 12px" }}>Status</th>
                        ...
                        <tr key={idx}>
                         <td>
                          <span style={{
                            ...
                            background: inv.billType === "POS" ? "#fff3e0" : "#e8f4fd",
                            color: inv.billType === "POS" ? "#e65100" : "#1565c0"
                          }}>
                            {inv.billType}
                          </span>
                        </td> 
                          ...
                          <td style={{ color: inv.balanceAmount > 0 ? "#e74c3c" : "#27ae60" }}>
                            ₹ {inv.balanceAmount?.toLocaleString()}
                          </td>
                          <td>
                            <span style={{
                              padding: "2px 10px", borderRadius: "12px", fontSize: "11px", fontWeight: "600",
                              background: inv.balanceAmount > 0 ? "#fdecea" : "#e6f4ea",
                              color: inv.balanceAmount > 0 ? "#d93025" : "#137333"
                            }}>
                              {inv.balanceAmount > 0 ? "Unpaid" : "Paid"}
                            </span>
                          </td>
                          <td>
                          <div className="d-flex" style={{ gap: "6px" }}>
                            ...
                            <button
                              ...
                              title="View"
                              onClick={() => navigate(`/addsale/${inv.id}`)}
                            >
                            ...
                            {/* ── EDIT BUTTON (already wired) ── */}
                            {canEdit && (
                              <button
                                className="btn btn-sm btn-light"
                                title="Edit"
                                onClick={() => navigate(`/editsale/${inv.id}`)}
                              >
              ...
              <div>
                ...
                <div className="d-flex justify-content-between align-items-start mb-3 pb-3" style={{ borderBottom: "1px solid #eee" }}>
                  ...
                  <div className="text-right">
                    <h5 style={{ fontWeight: 700, color: "#d93025" }}>{viewInvoice.billType} INVOICE</h5>
                ...
                {/* PARTY DETAILS */}
                <div className="mb-3">
                  <p style={{ fontSize: 12, color: "#888", marginBottom: 2 }}>BILLED TO</p>
                ...
                <div className="d-flex justify-content-end">
                  <table style={{ fontSize: 13, minWidth: 220 }}>
                    <tbody>
                      ...
                      <tr><td className="text-muted pr-3">Received</td><td className="text-right">₹{viewInvoice.amountReceived}</td></tr>
                      <tr>
                        <td className="text-muted pr-3">Balance</td>
                        <td className="text-right" style={{ color: viewInvoice.balanceAmount > 0 ? "#e74c3c" : "#27ae60", fontWeight: 600 }}>
...
export default ViewSaleInvoices;