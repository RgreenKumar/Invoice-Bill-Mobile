...
const ViewCustomers = () => {
  ...
  const canEdit   = getPermission(MODULES.CUSTOMER, "canEdit");
  const canDelete   = getPermission(MODULES.CUSTOMER, "canDelete");
  ...
  const [balanceMap,       setBalanceMap]       = useState({});  // ← add this
  ...
  const fetchCustomers = async () => {
    try {
      setLoading(true);
      const res = await axios.get(`${baseUrl}/getCustomers`, {   // ← changed
  ...
  const fetchAllBalances = async (customerList) => {
  const map = {};
  await Promise.all(
  ...
  if (result.isConfirmed) {
    try {
      ...
      fetchTransactions(selectedCustomer.name);
      fetchAllBalances(customers);
    } catch (err) {
  ...
  const handleCustomerClick = (customer) => {
    ...
    setTransactions([]);
    fetchTransactions(customer.name);
  };
  ...
  const totalBalance = transactions.reduce(
    (sum, t) => sum + (Number(t.balanceAmount) || 0), 0
  );

  // ── Total grand total from transactions ──
  const totalGrandTotal = transactions.reduce(
    (sum, t) => sum + (Number(t.grandTotal) || 0), 0
  );
                ...
                <div className="invoice-left-panel">
                  ...
                  <div className="invoice-left-panel-header">
                    <div className="vt-left-header-inner">
                      <h6 className="invoice-left-panel-title">Customers</h6>  {/* ← changed */}
                      <span className="invoice-count-badge">{filteredCustomers.length}</span>
                    </div>
                  </div>
                  ...
                  <div className="invoice-search-box">
                    <div className="input-group input-group-sm">
                      ...
                      <input
                        ...
                        placeholder="Search customer name..."  // ← changed
                        value={searchQuery}
                  ...
                  <div className="invoice-list-scroll">
                    {loading ? (
                      ...
                      <div className="text-center p-4" style={{ color: "#aaa", fontSize: 13 }}>
                        No customers found  {/* ← changed */}
                      </div>
                    ) : (
                     filteredCustomers.map((customer) => {
                      const isSelected = selectedCustomer?.id === customer.id;  // ← keep this
                      const displayBalance = isSelected
                        ? totalBalance
                        : (balanceMap[customer.id] ?? 0);  // ← changed this
                      return (
                        <div
                          key={customer.id}
                        ...
                        >
                            ...
                            <span style={{
                              fontSize: 13,
                              fontWeight: 600,
                              color: displayBalance > 0 ? "#e74c3c" : "#27ae60"
                            }}>
                  ...
                  <div className="invoice-right-panel-header">
                    <div className="vt-right-header-left">
                      <h6 className="invoice-right-panel-title">
                        {selectedCustomer ? selectedCustomer.name : "Customers"}  {/* ← changed */}
                      </h6>
                      {selectedCustomer &&  canEdit && (
                        <i
                          className="fa-solid fa-pen vt-edit-pen-icon"
                          style={{ cursor: "pointer", marginLeft: 8 }}
                          onClick={() => navigate(`/editCustomer/${selectedCustomer.id}`)}  // ← changed
                        ></i>
                      )}
                    ...
                    <div className="vt-right-header-right" style={{ display: "flex", alignItems: "center", gap: 8 }}>
                      <select
                        className="form-control form-control-sm invoice-filter-select"
                        value={filterOption}
                      ...
                      {/* // REPLACE WITH */}
                      {canCreate && (
                        <button className="btn btn-danger btn-sm" onClick={() => navigate("/addCustomer")}>
                          <i className="fa-solid fa-plus mr-1"></i> Add Customer 
                        </button>
                      )}
                    </div>
                  </div>
                  ...
                  {selectedCustomer ? (
                    <>
                      ...
                      <div style={{ display: "flex", gap: 12, padding: "12px 16px 0 16px" }}>
                        <div style={{
                          flex: 1, background: "#f0f7ff", borderRadius: 8,
                          padding: "10px 14px", borderLeft: "3px solid #1a73e8"
                        }}>
                          <div style={{ fontSize: 11, color: "#888", marginBottom: 2 }}>TOTAL SALES</div>  {/* ← changed */}
                        ...
                        <div style={{
                          flex: 1, background: totalBalance > 0 ? "#fff0f0" : "#f0fdf4",
                          borderRadius: 8, padding: "10px 14px",
                          borderLeft: `3px solid ${totalBalance > 0 ? "#e74c3c" : "#27ae60"}`
                        }}>
                        ...
                        <div style={{
                          ...
                          padding: "10px 14px", borderLeft: "3px solid #aaa"
                        }}>
                      ...
                      <div className="invoice-details-section">
                        <div className="invoice-details-header">
                          ...
                          <div className="vt-details-icons-row">
                            <i
                              className="fa-solid fa-magnifying-glass invoice-icon"
                            ...
                            <i
                              className="fa-solid fa-rotate-right invoice-icon"
                              style={{ cursor: "pointer" }}
                              onClick={() => fetchTransactions(selectedCustomer.name)}
                            ></i>
                          </div>
                        </div>
                                  ...
                                  <tr key={idx}>
                                    <td>
                                      <span style={{
                                        ...
                                        background: txn.billType === "SALE"     ? "#e8f4fd"
                                                  ...
                                                  : txn.billType === "ESTIMATE" ? "#f0fdf4"
                                                  : "#f5f5f5",
                                        color:     txn.billType === "SALE"     ? "#1a73e8"
                                                  : txn.billType === "POS"      ? "#e65100"
                                                  : txn.billType === "ESTIMATE" ? "#137333"
                                                  : "#555",
                                      }}>
                                        {txn.billType || "—"}
                                      </span>
                                    ...
                                    <td>
                                      <span style={{
                                       ...
                                       {txn.billType === "ESTIMATE" ? "—" : (isPaid ? "Paid" : "Unpaid")}
                                      </span>
                                    </td>
                                    <td style={{ position: "relative" }}
                                      ref={openMenuId === idx ? tableMenuRef : null}>
                                      <button
                              ...
                              <tfoot>
                                <tr style={{ background: "#f5f5f5", fontWeight: 700 }}>
                                  ...
                                  <td style={{ padding: "8px 12px", fontSize: 13 }}>
                                    <i className="fa-solid fa-indian-rupee-sign" style={{ fontSize: "0.7rem", marginRight: 2 }}></i>
                                    {filteredTxns.reduce((s, t) => s + (Number(t.grandTotal) || 0), 0)
                                      .toLocaleString("en-IN", { minimumFractionDigits: 2 })}
                                  </td>
                                  <td style={{ padding: "8px 12px", fontSize: 13, color: totalBalance > 0 ? "#e74c3c" : "#27ae60" }}>
                                    <i className="fa-solid fa-indian-rupee-sign" style={{ fontSize: "0.7rem", marginRight: 2 }}></i>
                                    {filteredTxns.reduce((s, t) => s + (Number(t.balanceAmount) || 0), 0)
                                      .toLocaleString("en-IN", { minimumFractionDigits: 2 })}
                                  </td>
                                  <td colSpan={2}></td>
                                </tr>
                    ...
                    <div className="invoice-empty-state">
                      <i className="fa-solid fa-user"></i>  {/* ← changed icon */}
                      <p>Select a customer to view details</p>  {/* ← changed */}
                    </div>
                  )}
                </div>
