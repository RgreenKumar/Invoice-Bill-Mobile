  ...
  const MySwal = withReactContent(Swal);
   ...
   const [fullsearch,setfullsearch]=useState(false);
    ...
    const searchUsers = async () => {
      try {
        const response = await axios.get(`${baseUrl}/admin/searchCashier`, {
          headers:{
            'Authorization':token
        },
          params: {
        ...
        gstinNumber: encodeURIComponent(gstin),
            companyName,
            page:currentPage,
            size:10
          }
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
          try {
            ...
            const response = await axios.get(`${baseUrl}/ViewAll/Cashiers`,{
              headers:{
                Authorization:token
              }
            });
     ...
     const renderPaginationButtons = () => {
      ...
      const handleDelete=async(username,email)=>{
        MySwal.fire({
            ...
            text: `Are you sure you want to Delete Cashier ${username}`,
            icon: "warning",
            ...
            if (result.isConfirmed) {
              try {
                if (email != null) {
                  const response = await axios.delete(`${baseUrl}/Delete/Cashier/${email}`,{
                  ...
                  if (response.status===200) {
                    MySwal.fire({
                      title: "Deleted",
                      text: `Cashier ${username} Deleted successfully`,
                      icon: "success",
      ...
      const handleDeactivate = async (userId, username, email) => {
        ...
        MySwal.fire({
          title: "De Activate ?",
          text: `Are you sure you want to DeActivate Cashier ${username}`,
          icon: "warning",
          showCancelButton: true,
          confirmButtonColor: "#d33",
          ...
          if (result.isConfirmed) {
            try {
              if (userId != null) {
                const response = await axios.delete(`${baseUrl}/admin/deactivate/cashier`, {
                  data: formData,
                  headers: {
                    'Authorization': token
                  }
                ...
                if (response.status === 200) {
                  MySwal.fire({
                    title: "De Activated",
                    text: `Cashier ${username} De Activated successfully`,
                    icon: "success",
      ...
      const handleActivate =async (userId,username,email)=>{
        ...
        MySwal.fire({
          title: " Activate ?",
          text: `Are you sure you want to Activate Cashier ${username}`,
          icon: "warning",
          showCancelButton: true,
          confirmButtonColor: "#d33",
          ...
          if (result.isConfirmed) {
            try {
              if (userId != null) {
                const response = await axios.delete(`${baseUrl}/admin/activate/cashier`,{
                 data:formData,
                  headers: {
                    'Authorization': token
                  }
                ...
                if (response.status===200) {
                  MySwal.fire({
                    title: "Activated",
                    text: `Cashier ${username}  Activated successfully`,
                    icon: "success",
                    confirmButtonText: "OK",
                }).then(() => {
                    fetchData();
        ...
        <table className="table table-hover table-bordered table-sm">
          ...
          {filterData().map((user, index) => (
              <tr key={user.userId}>
                ...
                <td className='py-2'>{user.username || "NA"}</td>
                <td className='py-2'>{user.email || "NA"}</td>
                <td className='py-2'>{user.companyName || "NA"}</td>
                <td className='py-2'>{user.phone || "NA"}</td>
                <td className='py-2'>{user.gstinNumber || "NA"}</td>
                <td className='py-2'>{user.dob || "NA"}</td>
