...
class _AddItemPageState extends State<AddItemPage>
  ...
  void _submitData() {
    final settings = appFeatureSettings.value;

    if (_nameController.text.trim().isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Item Name is required field!'),
    ...
    final newItem = Item(
      ...
      hsn: settings.enableGst && settings.enableHsn
          ? _hsnController.text.trim()
          : '',
      unit: settings.itemsUnit ? _selectedUnit : 'NONE',
      category: settings.itemCategory ? _selectedCategory : 'None',
      code: _codeController.text.trim(),
      salePrice: enteredPrice,
      taxStatus: settings.enableGst && settings.itemWiseTax
          ? _selectedTaxStatus
          : 'Without Tax',
    );
    ...
    widget.onSaveItem(
      newItem,
    ); // Bubbles new data item structure state to main layout
  }
  ...
  Widget build(BuildContext context) {
    return ValueListenableBuilder<AppFeatureSettings>(
      ...
      builder: (context, settings, _) {
        if (!settings.enableItems) {
          return _disabledItemScreen();
        }

        final showHsn = settings.enableGst && settings.enableHsn;
        final showTax = settings.enableGst && settings.itemWiseTax;
                ...
                child: SingleChildScrollView(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      SizedBox(
                        height: settings.stockMaintenance ? 150 : 100,
                        child: TabBarView(
                          controller: _tabController,
                          children: [
                            Column(
                            ...
                            settings.stockMaintenance
                                ? const Center(
                                    child: Text('Stock Configuration Screen'),
                                  )
                                : const Center(
                                    child: Text(
                                      'Stock maintenance is disabled in settings',
                                    ),
                                  ),
                          ],
                        ),
              ...
              Container(
                ...
                child: Row(
                  ...
                  children: [
                    ...
                    ElevatedButton(
                      style: ElevatedButton.styleFrom(
                        backgroundColor: const Color(0xFF132E47),
                      ),
  ...
  Widget _disabledItemScreen() {
    return Scaffold(
      ...
      body: const Center(
        child: Text(
          'Item module is disabled. Enable Item in Settings to add items.',
        ),
      ),
    );
  }
