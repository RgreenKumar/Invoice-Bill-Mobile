...
class _ViewTrainerListScreenState extends State<ViewTrainerListScreen> {
  ...
  Future<void> _fetchSuppliers() async {
    ...
    try {
      ...
      // UX FIX: previously auto-selected `list.first` here, which meant
      // opening "Suppliers" always jumped straight into whichever
      // supplier happened to be first, hiding the actual supplier list -
      // reported as "it's automatically selecting the first customer"
      // (same bug, mirrored for suppliers). The person now always lands
      // on the list and taps to select.
    } catch (e) {
              ...
              if (isWide)
                Row(
                  ...
                  children: [
                    ...
                    // NOTE: no "Edit supplier" action here - the source
                    // React app (Trainer/AddSupplier.js) never supported
                    // editing an existing party, and there is no
                    // update-party endpoint in the backend to call. Add
                    // one here once that endpoint exists.
                  ],
                )
              else
