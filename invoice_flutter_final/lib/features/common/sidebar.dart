    ...
    return Container(
      ...
      child: SafeArea(
        child: Column(
          ...
          children: [
            // BUGFIX: the drawer's nav list used to start flush at the very
            // top of the safe area (right under the status bar), reported
            // as "the sidebar starts too high" / not aligned below the
            // header. This block gives it the same visual weight as
            // AppHeader (see its `preferredSize`) so the drawer content
            // reads as sitting below a header instead of floating at the
            // screen edge.
            Container(
              height: 64,
              ...
              child: const Row(
                children: [
                  ...
                  SizedBox(width: 8),
                  Text('InvoiceBill', style: TextStyle(color: Colors.white, fontSize: 16, fontWeight: FontWeight.w600)),
                ],
              ),
            ),
