...
## Session: 10-issue bug-fix pass (Sales/Estimate text corruption, calc bug, item search, Footer/POS removal, PDF download, low-stock, password toggle, sidebar layout)

1. **Text direction / calculations / item fetching (Sales + Estimate, one shared root cause).** `invoice_form_screen.dart` recreated a brand-new `TextEditingController` on every rebuild for Item Name / Qty / Price / Discount% instead of reusing one. Since Qty/Price called `setState` on every keystroke, the cursor reset to the start of the field after every character - which read as "text displaying right-to-left" (characters were being prepended, not rendered backwards), fed garbled digit order into `_calcRow` (explaining wrong totals), and sent corrupted keywords to `/searchItem` (explaining "items not being fetched"). Fixed by giving `_LineItem` persistent controllers created once and only ever updated via `.text =` (never recreated), properly disposed on row removal / bill reload / screen dispose. Also added 300ms debouncing to item/customer search (`Timer`-based), matching `AddSale.js`'s `itemSearchTimer`, to avoid the same race-condition class of bug.
2. **Sub Total formula bug (independent of #1).** The totals summary was summing `gross` (qty×price) for "Sub Total"; `AddSale.js` defines `subTotal` as the sum of `taxable` (post-discount, pre-tax). Fixed to match - Sub Total was previously inflated whenever any row had a discount.
3. **Update/Edit for Sale & Estimate.** Routes (`/editsale/:id`, `/editestimate/:id`) and the GET `/getBill/{id}` -> PUT `/updateBill/{id}` flow already existed and were already wired from the list screens' Edit buttons - this was almost certainly breaking only because of bug #1 corrupting the fields while editing. Should now work as a consequence of that fix.
4. **Footer removed.** `AppFooter` (rendered under every single screen via `MaterialApp.router`'s `builder`) deleted along with the now-unused `features/common/footer.dart`. (Settings > Footer Details - the admin config screen for what a footer *would* show - was left alone; it's a distinct settings page, not the always-visible bar.)
5. **POS page removed.** `/view/POSform` route and both sidebar "Invoice POS" links deleted. `Modules.invoicePos` permission tracking and the `billTypes=SALE,POS` filter / "POS" badge in the Sales list were deliberately left alone - removing those would break the Sales list's ability to show/filter bills that already have that bill type in the backend.
6. **Bill download was a non-functional stub** (documented as such in its own code comment): it fetched the real PDF bytes from `/downloadBill/{id}` but only ever showed a SnackBar *claiming* success, never actually saving or opening the file. Fixed using `Printing.sharePdf` (from the `printing` package, already a dependency for the print/PDF flow) to hand the real bytes to the native share/save/print sheet - for both the Sale invoice and Estimate list screens.
7. **Low-stock notifications never appearing - root cause found.** `GlobalStateProvider._fetchItemSettings()` (and the general/GST equivalents) *replaced* the cached settings map wholesale with whatever the backend returned, instead of merging onto the defaults. If a `/settings/item/get` response omitted `showLowStockDialog` for any reason, the key went from its default `true` to simply missing, and `itemSettings['showLowStockDialog'] == true` silently evaluated to `false` forever - hiding the header bell's low-stock badge and the notification-panel list regardless of actual stock levels. Fixed by merging fetched settings onto the defaults in all three fetchers and in the `setGeneralSettings`/`setGstSettings`/`setGlobalItemSettings` setters. Also hardened `/getLowStockItems`'s response parsing so an unexpected shape can't silently throw and get swallowed. (The React source has no *per-item* low-stock badge in the item list itself - only this header/notification mechanism - so nothing further was added there to avoid inventing UI beyond the source.)
8. **Password visibility toggle added** to Login (`login_screen.dart`) and both password fields on Forgot Password (`forgot_password_screen.dart`) - a `suffixIcon` eye/eye-off `IconButton` toggling `obscureText`, validation and existing auth logic untouched.
9. **Sidebar started flush at the very top of the screen** (no header/spacing before the first nav item, since `SafeArea` had nothing above the `SingleChildScrollView`). Added a 64px branded header block (matching `AppHeader`'s height, same icon/title) plus spacing, so the drawer content now visually sits below a header instead of floating at the screen edge.

## Session: Update-failure diagnostics, download discoverability, RenderFlex overflow

1. **"Failed to update" gave no way to tell why.** The catch block for
   both Save and Update showed the same generic "Failed to save bill.
   Please try again." regardless of what actually went wrong. Rewrote it
   to inspect the DioException's status code and response body and show
   a specific, actionable message (404 - the bill no longer exists /
   400 - shows the server's own validation message if it sends one /
   401-403 - not authorized / 5xx - server error / timeout - can't reach
   server). **This alone doesn't fix an unknown backend-side rejection**
   - the payload construction was already re-verified field-by-field
   against `AddSale.js`'s `updateBill` payload and matches - but it
   means the next time Update fails, the exact reason will be visible
   instead of a dead end. If it's still failing after this, the new
   message text is what's needed to pin down the real cause.
2. **Download "wasn't there" on mobile - it was, just hidden.** Both
   Sales and Estimate list screens' mobile card view only exposed
   View/Download/Edit/Delete via a long-press bottom sheet - not
   discoverable. Rewired both cards to show these as explicit icon
   buttons directly on the card at all times. Also added a Download
   button straight to the Invoice/Estimate form screen's AppBar when
   viewing/editing an existing bill, so it's not only reachable from the
   list.
3. **RenderFlex overflow (18px, right edge).** Root cause: the old
   mobile card used a bare `ListTile` with an unconstrained `trailing`
   Column next to a `title`/`subtitle` that could overflow for a long
   party name. Rewritten with the party name/subtitle wrapped in
   `Expanded` + `TextOverflow.ellipsis`, which is what the redesign
   above already needed anyway - fixes both problems in the same change,
   for both Sales and Estimate list screens.
...
## Session: Update 500s, download 500s, notification overflow, Noto-font warning

1. **Invoice/Estimate update & download 500s.** Re-verified `PUT
   /updateBill/{id}`, `GET /downloadBill/{id}`, and the update payload
   field-by-field against `AddSale.js`/`AddEstimate.js` again - endpoint,
   method, and payload shape were already an exact match, so nothing
   there was the divergence. Found and fixed two real client-side bugs
   instead:
   - `api_client.dart`'s `BaseOptions.headers` set
     `Content-Type: application/json` on **every** request, including
     bodyless GET/DELETE calls like `/downloadBill/{id}` - axios never
     does this (it only sets Content-Type when a request actually has a
     JSON `data` payload). A backend body-parsing filter keyed off
     Content-Type can throw on an empty body when that header is
     present, surfacing as a generic 500. Replaced the static header
     with an interceptor that only attaches `Content-Type:
     application/json` when `options.data` is present and isn't
     `FormData` (file-upload requests keep managing their own multipart
     boundary header exactly as before - untouched).
   - Added an `idParam()` helper (`lib/core/api/api_client.dart`) that
     normalizes any id to a clean integer string before it's
     interpolated into a URL, guarding against a bill id ever decoding
     from JSON as e.g. `68.0` and producing `/updateBill/68.0`, which a
     numeric-only route matcher would reject with an error that reads
     exactly like a routing/"no endpoint" failure. Applied to
     `getBill`/`updateBill`/`downloadBill` in `invoice_form_screen.dart`
     and `deleteBill`/`downloadBill` in both list screens
     (`view_invoice_sale_screen.dart`, `estimate_quotation_screen.dart`).
2. **Notification panel `RenderFlex overflowed`.** Two independent
   causes in `notification_panel.dart`:
   - The header row ("Notifications" title + "mark as read"/"clear all"
     buttons) had no `Expanded`/`Flexible` inside the panel's fixed
     340px width (`header.dart`'s `SizedBox(width: 340, ...)`) - wrapped
     the title in `Expanded` with ellipsis and tightened both
     `TextButton`s' padding/tap-target so the row can no longer exceed
     the available width.
   - Each notification's `ListTile` combined a multi-line (`maxLines:
     3`) title with a subtitle - `ListTile` sizes itself for a fixed
     single/two/three-line layout and doesn't cleanly support an
     arbitrarily-wrapping title plus a separate subtitle, which is what
     was actually throwing the internal RenderFlex overflow. Replaced it
     with a manually laid out `InkWell` > `Row` > `Expanded(Column(...))`
     that sizes to its actual wrapped content instead, so long messages
     now wrap fully (no more forced 3-line ellipsis) and the panel's
     outer `ListView` handles vertical scrolling as before.
3. **"Could not find a set of Noto fonts…" warning.** Root cause: raw
   emoji glyphs (`⚠️`) used directly in two `Text` widgets in
   `notification_panel.dart` - the only emoji usage anywhere in the
   codebase. `google_fonts` (Open Sans/Playfair, used throughout the
   theme) has no emoji glyphs, so Skia searches for a system Noto
   fallback and warns every time it can't find one. Replaced both with
   `Icon(Icons.warning_amber_rounded, ...)`, which renders from the
   bundled Material icon font instead of requiring a system emoji font.

**Verification performed** (no Flutter/Dart SDK available in this
environment to run `flutter analyze`/`flutter test`): manual line-by-line
diff of every touched endpoint/payload against the React source, a
brace/paren balance check on every edited file, and a full-codebase
regex sweep confirming no other emoji glyphs remain anywhere in `lib/`.
Recommend running `flutter analyze` and exercising the five flows (sale
update, estimate update, sale download, estimate download, notification
bell) once in a real environment before shipping, since that couldn't be
done here.
