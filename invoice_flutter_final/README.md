...
> Estimates) and the Settings module (general, mail, footer, role display
> names, weightage, taxes & GST, item settings, manage users/cashiers) are
> now functional. Several real navigation/UX bugs are fixed
> (blank-screen-after-save, missing Add Supplier button, disorienting
> auto-navigation in the Items/Customers/Suppliers sections), and
> AddCustomer/AddSupplier were rebuilt fully mobile-first.**
> `InvoicePOS.js` was explicitly excluded from this pass per request.
> See `CONVERSION_STATUS.md` for the full breakdown.
